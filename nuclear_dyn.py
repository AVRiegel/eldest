...
if args.fc:     # If an FC input file is provided, FC integrals will be read from it and their calculation skipped
    ...
    ##### If resonance state has Morse potential
    if (res_pot_type == 'morse'):
        ...
        #            outfile.write(f'm = {m}, res_fin = {FC: 10.10E}, |res_fin| = {np.abs(FC):10.10E}\n')   #?
                    if partial_GamR:
                        FC = FCfunc(m,fin_a,fin_Req,fin_de,red_mass,
                                    ...
                                    V_of_R=lambda R: 1)
                        parttmp.append(FC)
                        print(f'm = {m}, res_fin_woVR = {FC: 10.10E}, |res_fin_woVR| = {np.abs(FC):10.10E}')   #?
        #               outfile.write(f'm = {m}, res_fin_woVR = {FC: 10.10E}, |res_fin_woVR| = {np.abs(FC):10.10E}\n')   #?
                res_fin.insert(0,tmp)
